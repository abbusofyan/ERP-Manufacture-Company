<template>
    <Layout>
        <div class="flex flex-wrap items-center gap-13 mb-26">
            <div v-if="contract" class="big-title">Edit Project Contract</div>
            <div v-else class="big-title">New Project Contract</div>
            ...
            <nav class="breadcrumbs">
                <ol>
                    ...
                    <li>
                        <span v-if="contract">Edit Project Contract</span>
                        <span v-else>Create Project Contract</span>
                    </li>
                </ol>
            </nav>
        </div>
                    ...
                    <div class="boxes">
                        ...
                        <div class="box border-0 border-b-[1px] border-solid border-[#EBE9F1] mb-[5.6rem]">
                            ...
                            <div class="form-group">
                                <label>Project Contract Number</label>
                                <input class="form-control disabled" type="text" :class="{ 'is-invalid': form.errors.project_contract_number }" v-model="form.project_contract_number" placeholder="0" disabled>
                                <div v-if="form.errors.project_contract_number" class="invalid-feedback d-block">{{ form.errors.project_contract_number }}</div>
                            </div>
                            <div class="grid md:grid-cols-2 gap-[7.7rem]">
                                <div class="form-group">
                                    ...
                                    <VueDatePicker
                                        ...
                                        :model-value="form.start_duration_date" @update:model-value="updateContractPeriod(); updateMaintainProject()"
                        ...
                        <div class="mb-[5.6rem]">
                            <div class="form-group">
                                <label>Number of Project</label>
                                <Select2
                                    v-model="numberOfProjects"
                                    :options="[
                                                    {
                                                        'id' : 1,
                                                        'text' : '1 Service'
                        ...
                        <div class="box border-0 border-b-[1px] border-solid border-[#EBE9F1] mb-[5.6rem]">
                            ...
                            <div class="border-0 border-b-[1px] border-solid border-[#EBE9F1] mb-[5.6rem]">
                                <div class="grid md:grid-cols-2 gap-[7.7rem]">
                                    <div>
                                        <template v-for="(item, index) in form.maintain_projects" :key="index">
                                            <div class="form-group" v-if="item.is_active">
                                                ...
                                                <div v-if="form.errors[`maintain_projects.${index}.date`]" class="invalid-feedback d-block">{{ form.errors[`maintain_projects.${index}.date`] }}</div>
                                            </div>
                                        </template>
                                        <div v-if="form.errors.maintain_projects" class="invalid-feedback d-block">{{ form.errors.maintain_projects }}</div>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
...
if (props.contract) {
    const {
        project_contract_number, start_duration_date, end_duration_date, value, term_of_payment, customer_id, vehicles,
    ...
    form.project_contract_number = project_contract_number;
    ...
    () => {
        ...
        form.pricing.forEach((pricing, index) => {
            const existingDetail = existingDetails.find(
                detail => detail.project_contract_pricing_id === pricing.id
            );

            form.vehicle_item_details.push({
                id: index,
                project_contract_pricing_id: pricing.id,
                pricing: pricing,
...
const updateContract = () => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            if (props.contract) {
                form.put(`/project-contracts/${props.contract.id}`);
            } else {
                form.post('/project-contracts');
            }
        }
    });
};
...
const calculateMaintenanceDates = (startDate, numberOfProjects) => {
    const start = new Date(startDate);
    const startMonth = start.getMonth(); // Get the starting month (0-11)
    const startYear = start.getFullYear(); // Get the starting year
    let projectMonths = [];

    // Apply the maintenance frequency rules based on the number of projects per year
    switch (Number(numberOfProjects ?? 1)) {
        case 1:
            projectMonths = [startMonth + 5]; // 1 Project: +6 months
            break;
        case 2:
            projectMonths = [startMonth + 4, startMonth + 10]; // 2 Projects: +5 months, +11 months
            break;
        case 3:
            projectMonths = [startMonth + 2, startMonth + 6, startMonth + 10]; // 3 Projects: +3 months, +7 months, +11 months
            break;
        case 4:
            projectMonths = [startMonth + 1, startMonth + 4, startMonth + 7, startMonth + 10]; // 4 Projects: +2, +5, +8, +11 months
            break;
        case 5:
            projectMonths = [startMonth + 1, startMonth + 4, startMonth + 6, startMonth + 8, startMonth + 10]; // 5 Projects: +2, +5, +7, +9, +11 months
            break;
        case 6:
            projectMonths = [startMonth + 1, startMonth + 3, startMonth + 5, startMonth + 7, startMonth + 9, startMonth + 10]; // 6 Projects: +2, +4, +6, +8, +10, +11 months
            break;
        default:
            projectMonths = [startMonth]; // Default case if no valid number of projects is provided
            break;
    }

    // Calculate actual maintenance month/year, adjust for overflow of months (if > 12)
    return projectMonths.map(monthOffset => {
...
const updateMaintainProject = () => {
    if (form.start_duration_date) {
        const start = new Date(form.start_duration_date);
        const dates = calculateMaintenanceDates(start, numberOfProjects.value);

        for (let i = 0; i < Number(numberOfProjects.value ?? 1); i++) {
            console.log(dates[i])
            form.maintain_projects[i]['date'] = dates[i] || null;
        }
    }
}
...
watch(numberOfProjects, (newVal) => {
    if (newVal >= form.maintain_projects.length) {
        form.maintain_projects.forEach((item, index) => {
            form.maintain_projects[index]['is_active'] = true;
        })
