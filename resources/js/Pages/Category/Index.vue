<template>
    <Layout>
        <div class="flex flex-wrap items-center gap-13 mb-26">
            <div class="big-title">Category List</div>
        ...
        <div class="box rounded-md shadow-default bg-white">
            <div class="flex flex-wrap gap-16 justify-between py-20 px-25 gap-1">
                ...
                <div class="flex flex-wrap gap-16 justify-between">
                    ...
                    <div class="btn-group">
                        <Link class="btn btn-purple" href="/categories/create">Add New Category</Link>
                    </div>
                </div>
            </div>
            <div class="table-responsive">
                <table class="table select-rows">
                    ...
                    <tr>
                        ...
                        <th :class="{ sorting_asc: order == 'prefix' && by == 'asc', sorting_desc: order == 'prefix' && by == 'desc' }" @click="sort('prefix')">
                            <div class="flex items-center justify-between gap-1">
                                <span>Prefix</span><span class="sort-ic"><img src="../../../assets/img/sort.svg" alt="sort"></span>
                            </div>
                        </th>
                    ...
                    <tr v-for="category in categories.data" :key="category.id">
                        ...
                        <td>
                            {{ $filters.formatDateTime(category.created_at) }}
                        </td>
                        <td>
                            <div class="custom-checkbox style-1">
                                <input type="checkbox" disabled :checked="!category.deleted_at ? true : false">
                                <label></label>
                            </div>
                        </td>
                        <td>
                            <div class="el-actions">
                                <Link v-if="!category.deleted_at"  class="text-green" :href="`/categories/${category.id}/edit`"><em class="fa-regular fa-pen-to-square"></em></Link>
                                <a v-if="!category.deleted_at" class="text-red" @click="destroy(category.id)" href="javascript:void(0)"><em class="fa-regular fa-eye-slash"></em></a>
                                <a v-if="category.deleted_at" class="text-green" @click="restore(category.id)" href="javascript:void(0)"><em class="fa-regular fa-eye"></em></a>
                            </div>
                        </td>
                    </tr>
                    </tbody>
            ...
            <div class="flex items-center justify-between gap-26 mt-25 pb-25 px-25">
                <p>Showing {{ categories.from }} to {{ categories.to }} of {{ categories.total }} entries</p>
...
const restore = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            Inertia.get(`/categories/restore/${id}`, {
                preserveScroll: true,
            });
        }
    });
