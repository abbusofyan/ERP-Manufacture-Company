<template>
    <Layout>
        <div class="flex flex-wrap items-center gap-13 mb-26">
            <div class="big-title">Production Orders Schedule</div>
            ...
            <nav class="breadcrumbs">
                <ol>
                    ...
                    <li>
                        <Link href="/production-orders">Production Orders</Link>
                    </li>
                    <li>
                        <span>Schedule</span>
                    </li>
                ...
                <div class="col-lg-4">
                    <div class="table-responsive pb-0" style="min-height: auto">
                        <table class="table select-rows" style="min-width: auto">
                            ...
                            <tbody>
                            <tr @click="setProductionOrderId(order)" class="cursor-pointer" v-for="order in productionOrders.data" :class="{ 'row-active' : order.id == production_order_id }" :key="order.id">
                    ...
                    <div class="flex flex-column items-center justify-between gap-26 mt-25 pb-25 px-25">
                        <p>Showing {{ productionOrders.from }} to {{ productionOrders.to }} of {{ productionOrders.total }} entries</p>
...
const production_order_id = ref(props.filters.production_order_id);

const timelogDates = computed(() => {
    if (!props.productionOrder || !props.productionOrder.processes) {
        return [];
    }

    const datesSet = new Set();

    props.productionOrder.processes.forEach(process => {
        process.timelogs.forEach(timelog => {
            if (timelog.start_time) { // Kiểm tra timelog có start_time
                const dateStr = timelog.start_time.split(' ')[0];
                datesSet.add(dateStr);
            }
            if (timelog.end_time) { // Kiểm tra timelog có end_time
...
const filter = () => {
    Inertia.get(
        '/production-orders-schedule',
        {
            ...
            production_order_id: production_order_id.value,
        },
        {
            preserveState: true,
            preserveScroll: true,
...
watch(
    [() => order.value, () => by.value, () => paginate.value, () => production_order_id.value],
    () => {
        filter();
    }
);
...
const setProductionOrderId = (order) => {
    production_order_id.value = order.id;
};
...
const startOfDay = computed(() => {
    ...
    props.productionOrder.processes.forEach(process => {
        process.timelogs.forEach(timelog => {
            if (!timelog.start_time) return; // Skip if no start_time
...
const endOfDay = computed(() => {
    ...
    props.productionOrder.processes.forEach(process => {
        process.timelogs.forEach(timelog => {
            if (!timelog.end_time) return; // Skip if no end_time
...
const processedProcesses = computed(() => {
    ...
    return props.productionOrder.processes
        .map((process) => {
            ...
            timelogs.forEach((timelog, index) => {
                if (!timelog.start_time || !timelog.end_time) return; // Skip if missing times
            ...
            activePeriods.forEach((period) => {
                for (let j = 0; j < timeSlots.value.length; j++) {
                    ...
                    if (overlap > 0) {
                        ...
                        let color = '#6A9C89'; // Default Working Time color
                        if (period.type === 'ot') {
                            color = '#A04747'; // OT Time color
                        }
