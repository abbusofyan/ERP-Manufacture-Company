<template>
    <Layout>
        <div class="flex flex-wrap items-center gap-13 mb-26">
            <div class="big-title">Service Appointments</div>
            ...
            <div class="flex flex-wrap gap-16 justify-between py-20 px-25 gap-1">
                ...
                <div class="flex flex-wrap gap-16 justify-between">
                    ...
                    <div class="btn-group" v-if="permissions.includes('create-service_appointment')">
                        <Link class="btn btn-purple" href="/service-appointments/create">Add New Appointment</Link>
                    </div>
                </div>
            </div>
                            ...
                            <div class="more">
                                ...
                                <ul class="more-panel">
                                    <li v-if="permissions.includes('update-service_appointment')">
                                        <Link class="text-purple" :href="`/service-appointments/${appointment.id}`"><span class="icon"><img src="../../../assets/img/documents.svg" alt=""></span><span>View Detail</span></Link>
                                    </li>
                                    <li v-if="permissions.includes('update-service_appointment') && ![3, 4].includes(appointment.status) && !appointment.is_expired">
                                        <a href="javascript:void(0)" @click="convertServiceOrder(appointment.id)" class="text-nowrap text-warning"><span class="icon"><img src="../../../assets/img/checkbox.svg" alt=""></span><span>Convert to Service Order</span></a>
                                    </li>
                                    <li v-if="permissions.includes('update-service_appointment') && ![3, 4].includes(appointment.status)">
                                        <Link :href="`/service-appointments/${appointment.id}/edit`"><span class="icon"><img src="../../../assets/img/edit.svg" alt=""></span><span>Edit</span></Link>
                                    </li>
                                    <li v-if="permissions.includes('delete-service_appointment') && ![3, 4].includes(appointment.status)">
...
const convertServiceOrder = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            Inertia.post(`/service-appointments/${id}/convert-service-order`, {
                preserveState: true,
                preserveScroll: true,
                replace: true,
                onSuccess: () => {
...
const destroy = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            Inertia.delete(`/service-appointments/${id}`, {
                preserveScroll: true,
                onSuccess: () => {
                    Swal.fire({
                            title: 'Success !',
