    ...
    <Layout>
        ...
        <div class="mb-[2.6rem]">
            <div class="stage-tab-2 d-flex flex-wrap gap-16">
                <Link v-for="(tab, index) in stages" class="btn btn-purple" :href="`/service-technicians?stage=${index}`" :class="{'btn-purple--brounded border-0': Number(index) !== Number(stage)}">
            ...
            <div class="table-responsive">
                <table class="table select-rows">
                    ...
                    <tr v-for="process in processes.data" :key="process.id">
                        <td>{{ process.service_order.customer.name }}</td>
                        <td>{{ process.service_order?.vehicle?.license_plate }}</td>
                        <td>{{ process.service_order?.technician?.name }}</td>
                        <td>
                            ...
                            <div class="more">
                                ...
                                <ul class="more-panel">
                                    <li v-if="permissions.includes('view-requisition')">
                                        <Link :href="`/service-orders/${process.service_order.id}/processes`"><span class="icon"><img src="../../../assets/img/documents.svg" alt=""></span><span>View</span></Link>
                                    </li>
                                    <li v-if="Number(process.status) === 1 || Number(process.status) === 3">
                                        <Link method="POST" :href="`/service-orders/${process.service_order.id}/processes/${process.id}/continue`"><span class="icon"><img src="../../../assets/img/play.svg" alt=""></span><span>Start Process</span></Link>
                                    </li>
                                    <li v-if="Number(process.status) === 2">
                                        <Link method="POST" :href="`/service-orders/${process.service_order.id}/processes/${process.id}/pause`"><span class="icon"><img src="../../../assets/img/player-pause.svg" alt=""></span><span>Pause Process</span></Link>
                                    </li>
                                    <li v-if="Number(process.status) === 2">
                                        <Link v-if="process.stage !== 1" method="POST" :href="`/service-orders/${process.service_order.id}/processes/${process.id}/complete`" class="text-success"><span class="icon"><img src="../../../assets/img/checkbox-green.svg" alt=""></span><span>Complete Process</span></Link>
                                ...
                                <DeliveryDateForm :serviceOrder="process.service_order" :additionId="process.id"></DeliveryDateForm>
                            </div>
                        </td>
                    </tr>
                    </tbody>
...
import DeliveryDateForm from "./../ServiceOrder/Delivery/DeliveryDateForm.vue";
