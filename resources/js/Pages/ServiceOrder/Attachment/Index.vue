<template>
    ...
    <Modal :serviceOrder="serviceOrder"></Modal>

    <div v-if="serviceOrder.stage !== 1" class="box pb-[5.6rem] rounded-md shadow-default bg-white mt-[2.6rem]">
        <div class="flex flex-wrap gap-16 justify-between py-20 px-25 gap-1">
            ...
            <div class="flex flex-wrap gap-16 justify-between">
                <div class="btn-group">
                    <a v-if="![7, 8, 9].includes(serviceOrder.status)" class="btn btn-purple" data-bs-toggle="modal" data-bs-target="#uploadAttachment" ref="openModal" href="javascript:void(0)">Upload file <i class="fa fa-upload"></i></a>
                </div>
            </div>
        </div>
        <div class="form-wrap">
    ...
    <div class="box pb-[5.6rem] rounded-md shadow-default bg-white mt-[2.6rem]">
        <div class="flex flex-wrap gap-16 justify-between py-20 px-25 gap-1">
            ...
            <div v-if="!serviceOrder.stage || (serviceOrder.stage && serviceOrder.stage === 1)" class="flex flex-wrap gap-16 justify-between">
                <div class="btn-group">
                ...
                <div class="table-responsive pb-0" style="min-height: unset">
                    <table class="table select-rows">
                        ...
                        <tr v-for="attachment in firstAttachments.data" :key="attachment.id">
                            <td>
                                <a class="text-purple" target="_blank" :href="attachment.full_url">
                                    {{ attachment.name }}
                                </a>
                            </td>
                            <td>{{ attachment.remarks }}</td>
                            <td>
                                <div v-if="serviceOrder.stage === 1" class="el-actions justify-end">
                                    <a @click="destroy(attachment.id)" href="javascript:void(0)"><span class="icon"><img src="../../../../assets/img/delete.svg" alt="delete"></span></a>
                                </div>
                            </td>
                        </tr>
...
const destroy = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            Inertia.delete(`/service-orders/${props.serviceOrder.id}/attachments/${id}`, {
                preserveScroll: true,
                onSuccess: () => {
                    Swal.fire({
                        title: 'Success!',
