    ...
    <div class="box pb-[5.6rem] rounded-md shadow-default bg-white mt-[2.6rem]">
        <div class="flex flex-wrap gap-16 justify-between py-20 px-25 gap-1">
            ...
            <div class="flex flex-wrap gap-16 justify-between">
                <div class="btn-group">
                    <a v-if="![7, 8, 9].includes(projectOrder.status)" class="btn btn-purple" data-bs-toggle="modal" data-bs-target="#uploadAttachment" ref="openModal" href="javascript:void(0)">Upload file <i class="fa fa-upload"></i></a>
                </div>
            </div>
        </div>
        <div class="form-wrap">
...
const destroy = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            Inertia.delete(`/project-orders/${props.projectOrder.id}/reports/${id}`, {
                preserveScroll: true,
                onSuccess: () => {
                    Swal.fire({
                        title: 'Success!',
