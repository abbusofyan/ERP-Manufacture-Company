    ...
    <Layout>
        ...
        <div class="box rounded-md shadow-default bg-white">
            ...
            <div class="table-responsive">
                <table class="table select-rows">
                    ...
                    <tr>
                        ...
                        <th class="text-nowrap">Service Order number</th>
                    ...
                    <tbody>
                    <tr v-for="invoice in serviceInvoices.data" :key="invoice.id">
                        <td>{{ invoice.code }}</td>
                        <td>{{ invoice.service_quotation.code }}</td>
                        <td>{{ invoice.service_quotation.service_order_type }}</td>
                        <td>{{ invoice.service_quotation.service_order?.code }}</td>
                        <td>{{ invoice.service_quotation.customer?.name }}</td>
                        <td>{{ invoice.service_quotation.customer?.code }}</td>
                        <td>{{ invoice.service_quotation.vehicle.license_plate }}</td>
                            ...
                            <div class="more">
                                ...
                                <ul class="more-panel">
                                    <li v-if="permissions.includes('update-service_appointment')">
                                        <Link :href="`/service-invoices/${invoice.id}`"><span class="icon"><img src="../../../assets/img/documents.svg" alt=""></span><span>View Detail</span></Link>
                                    </li>
                                    <li v-if="permissions.includes('update-service_appointment')">
                                ...
                                <Modal :serviceInvoice="invoice"></Modal>
                            </div>
                        </td>
                    </tr>
                    </tbody>
            ...
            <div class="flex items-center justify-between gap-26 mt-25 pb-25 px-25">
                <p>Showing {{ serviceInvoices.from }} to {{ serviceInvoices.to }} of {{ serviceInvoices.total }} entries</p>
                <Pagination :links="serviceInvoices.links"/>
            </div>
        </div>
    </Layout>
</template>
...
import Modal from "../ServiceInvoice/UploadForm.vue";

const permissions = usePage().props.value.auth.user.permissions;

const props = defineProps({
    serviceInvoices: Object,
    filters: Object,
});
