            ...
            <table class="table select-rows">
                ...
                <tbody>
                    <tr v-for="(process, index) in form.processes" :key="index" @click.stop="toggleDetailRow(index)">
                        <template v-if="process.department == 'Engineering' || process.is_child">
...
const toggleDetailRow = (index) => {
    ...
    if (selectedRow.value != null) {
        ...
        console.log(form_detail.groups);
...
const startProcess = (process) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            Inertia.get(`/engineering-order/start-process/${props.order.id}/${props.order.production_order.id}/${process.process_id}`);
        }
    });
};
