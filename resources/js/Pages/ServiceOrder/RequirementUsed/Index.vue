    ...
    <div class="box pb-[5.6rem] rounded-md shadow-default bg-white mt-[2.6rem]">
        <div class="flex flex-wrap gap-16 justify-between py-20 px-25 gap-1">
            ...
            <div class="flex flex-wrap gap-16 justify-between">
                ...
                <div class="btn-group">
                    <Link class="btn btn-purple" :href="`/service-orders/${serviceOrder.id}/requirements-used/live-stock-status`">
                        Live Stock Status <i class="fa fa-list"></i>
                    </Link>
                    <Link class="btn btn-purple" :href="`/service-orders/${serviceOrder.id}/requirements-used/requisitions`">
                        Item Status Detail <i class="fa fa-list"></i>
                    </Link>
                    <a v-if="serviceOrder.status != 7" class="btn btn-purple" data-bs-toggle="modal" data-bs-target="#uploadRequirementUsed" ref="openModal" href="javascript:void(0)">Add Item <i class="fa fa-plus"></i></a>
                    <a v-if="([7, 8, 9].includes(serviceOrder.status) || !serviceOrder.quotation_generated_after_update)" style="opacity: 0.5" class="btn btn-purple disabled" href="javascript:void(0)">
                        Generate Quotation <i class="fa fa-file"></i>
                    </a>
                    <Link v-else class="btn btn-purple" :href="`/service-orders/${serviceOrder.id}/requirements-used/generate-quotation`" method="post">
                        Generate Quotation <i class="fa fa-file"></i>
                    </Link>
                </div>
            </div>
                ...
                <div class="table-responsive pb-0" style="min-height: unset">
                    <table class="table select-rows">
                        ...
                        <tr v-for="item in requirementsUsed.data" :key="item.id">
                            ...
                            <td>{{ item.requisition_item.requested_qty }}</td>
                            <td>{{ item.requisition_item.issued_qty }}</td>
                            <td>{{ item.requisition_item.committed_qty }}</td>
                            <td>{{ item.requisition_item.returned_qty ?? 0 }}</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
...
const filter = () => {
    Inertia.get(
        `/service-orders/${props.serviceOrder.id}/requirements-used`,
        {
            search: search.value,
            order: order.value,
            by: by.value,
...
const destroy = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            Inertia.delete(`/service-orders/${props.serviceOrder.id}/requirements-used/${id}`, {
                preserveScroll: true,
                onSuccess: () => {
                    Swal.fire({
                        title: 'Success!',
