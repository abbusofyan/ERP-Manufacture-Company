<template>
    <Layout>
        <div class="flex flex-wrap items-center gap-13 mb-26">
            <div class="big-title">Project Contracts</div>
            ...
            <div class="flex flex-wrap gap-16 justify-between py-20 px-25 gap-1">
                ...
                <div class="flex flex-wrap gap-16 justify-between">
                    ...
                    <div class="btn-group" v-if="permissions.includes('create-project_appointment')">
                        <Link class="btn btn-purple" href="/project-contracts/create">Add New Contract</Link>
                    </div>
                </div>
            </div>
            <div class="table-responsive">
                <table class="table select-rows">
                    ...
                    <tr v-for="contract in contracts.data" :key="contract.id">
                        <td>{{ contract.project_contract_number }}</td>
                            ...
                            <div class="more">
                                ...
                                <ul class="more-panel">
                                    <li v-if="permissions.includes('update-project_appointment')">
                                        <Link class="" :href="`/project-contracts/${contract.id}`"><span class="icon"><img src="../../../assets/img/documents.svg" alt=""></span><span>View Detail</span></Link>
                                    </li>
                                    <li v-if="permissions.includes('update-project_appointment') && Number(contract.status) === 1">
                                        <Link :href="`/project-contracts/${contract.id}/edit`"><span class="icon"><img src="../../../assets/img/edit.svg" alt=""></span><span>Edit</span></Link>
                                    </li>
                                    <li v-if="permissions.includes('view-project_appointment')">
                                        <a :href="`/project-contracts/${contract.id}/download`"><span class="icon"><img src="../../../assets/img/download.svg" alt=""></span><span>Download</span></a>
                                    </li>
...
const submitImage = (id) => {
    form.post(`/project-contracts/${id}/upload-photo`, {
        onSuccess: () => {
            form.reset();
            Swal.fire({
                title: 'Success !',
...
const voidChange = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            Inertia.post(`/project-contracts/${id}/void`, {}, {
                onSuccess: () => {
                    form.reset();
                    Swal.fire({
                        title: 'Success !',
...
const destroy = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            Inertia.delete(`/project-contracts/${id}`, {
                preserveScroll: true,
                onSuccess: () => {
                    Swal.fire({
                            title: 'Success!',
                            text: "Project contract deleted successfully.",
                            icon: 'success',
                            showConfirmButton: false,
                            showCancelButton: true,
                            cancelButtonText: 'OK',
