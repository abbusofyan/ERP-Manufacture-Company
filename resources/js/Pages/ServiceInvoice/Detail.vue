    ...
    <Layout>
                    ...
                    <div class="box border-0 border-b-[1px] border-solid border-[#EBE9F1] mb-[2.6rem] pb-25">
                        <div class="grid md:grid-cols-2 gap-[7.7rem]">
                            ...
                            <div class="d-flex justify-content-end">
                                <div class="el-tag" :class="serviceInvoice.status_class">
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <div class="d-flex justify-content-end">
                                ...
                                <div class="text-right text-bold">
                                    <div class="mb-14 pl-24 ml-20">${{ serviceInvoice.subtotal_amount }}</div>
                                    <div class="mb-14 pl-24 ml-20">${{ serviceInvoice.discount_amount }}</div>
                                    <div class="mb-14 pl-24 ml-20">${{ serviceInvoice.tax_amount }}</div>
                                    <div class="border-0 border-b-[1px] border-solid border-[#EBE9F1] pb-17 mb-17"></div>
                                    <div class="mb-14 pl-24 ml-20">${{ serviceInvoice.total_amount }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap gap-16 justify-between mt-[5.6rem] border-0 border-t-[1px] border-solid border-[#EBE9F1] pt-20">
                        <div>
                            <div class="btn-group">
                                ...
                                <a class="btn btn-purple btn-purple--brounded" v-if="serviceInvoice.delivery_order_invoice_url" :href="serviceInvoice.delivery_order_invoice_url" target="_blank" download="PO-Invoice"><span>Download DO</span></a>
                                <a target="_blank" :download="serviceInvoice.name" class="btn btn-purple btn-purple--brounded" :href="serviceInvoice.file_url"><span>Download Invoice</span></a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="box rounded-md shadow-default bg-white mt-[2.6rem]">
            <div class="flex flex-wrap gap-16 justify-between py-20 px-25 gap-1">
                ...
                <a v-if="serviceInvoice.status !== 'Paid'" class="btn btn-purple" data-bs-toggle="modal" :data-bs-target="`#uploadPaid-${serviceInvoice.id}`" ref="openModal" href="javascript:void(0)">
                    <span>
                ...
                <Modal :serviceInvoice="serviceInvoice"></Modal>
            </div>
...
const serviceQuotation = props.serviceInvoice.service_quotation;
...
const submitForm = () => {
    form.put(`/service-invoices/${props.serviceInvoice.id}`, {
        preserveScroll: true,
        onSuccess: () => {
            Swal.fire({
                title: 'Success !',
...
const generatePO = () => {
    form.post(`/service-invoices/${props.serviceInvoice.id}/generate-delivery-order`, {
        preserveScroll: true,
        onSuccess: () => {
            Swal.fire({
                title: 'Success !',
...
onMounted(() => {
    form.finance_amount = props.serviceInvoice.finance_amount ?? 0;
    form.deposit_required = props.serviceInvoice.deposit_required ?? 0;
    form.rounding = props.serviceInvoice.rounding ?? 2;
    form.freight_charges = props.serviceInvoice.freight_charges;
    form.customer_reference_number = props.serviceInvoice.customer_reference_number;
    form.amend_customer_address = props.serviceInvoice.amend_customer_address;
    form.footer = props.serviceInvoice.footer;
})
</script>
