<template>
    <Layout>
        <div class="flex flex-wrap items-center gap-13 mb-26">
            <div class="big-title">Service Orders Schedule</div>
                ...
                <div class="col-lg-4">
                    <div class="table-responsive pb-0" style="min-height: auto">
                        <table class="table select-rows" style="min-width: auto">
                            ...
                            <tbody>
                            <tr @click="setOrderId(order)" class="cursor-pointer" v-for="order in serviceOrders.data" :class="{ 'row-active' : order.id == service_order_id }" :key="order.id">
...
const service_order_id = ref(props.filters.service_order_id);

// Extract all unique dates from timelogs
const timelogDates = computed(() => {
    if (!props.serviceOrder || !props.serviceOrder.service_order_process) {
        return [];
    }

    const datesSet = new Set();

    props.serviceOrder.service_order_process.forEach(process => {
...
const filter = () => {
    Inertia.get(
        '/service-orders-schedule',
        {
            ...
            service_order_id: service_order_id.value,
        },
        {
            preserveState: true,
            preserveScroll: true,
...
watch(
    [() => order.value, () => by.value, () => paginate.value, () => service_order_id.value],
    () => {
        filter();
    }
);
...
const setOrderId = (order) => {
    service_order_id.value = order.id;
};
...
const processedProcesses = computed(() => {
    ...
    return props.serviceOrder.service_order_process
        .map((process) => {
