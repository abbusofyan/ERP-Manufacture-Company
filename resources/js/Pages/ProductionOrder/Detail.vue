        ...
        <form @submit.prevent="assembly ? form.put(`/assembly/${order.id}`) : form.post('/assembly')">
            <div class="box pt-26 px-24 pb-[5.6rem] rounded-md shadow-default bg-white">
                <div class="text-18 font-medium pb-17 mb-17 border-0 border-b-[1px] border-solid border-[#EBE9F1] flex justify-between gap-3 items-center">
                    <span>Production Order Detail</span>
                </div>
                <div class="row">
                    <div class="col">
                        <table class="table-1-el w-full">
                            <tbody>
                                <tr>
                                    ...
                                    <td class="pl-15"> : {{order.quotation.code}}</td>
                                </tr>
                                <tr>
                                    <th class="text-nowrap">Production Order No</th>
                                    <td class="pl-15"> : {{ order.code }}</td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <td class="pl-15"> : {{ order.category }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
import Tab from "@/Pages/ProductionOrder/Tab.vue";
...
onMounted(() => {
    if (props.order) {
        let specs = props.order.quotation.specs
        ...
        form.summaries = props.order.process_timelogs.map(log => {
            return {
                'id': log.id,
