        ...
        <div class="box pt-20 px-25 pb-[5.6rem] rounded-md shadow-default bg-white">
            ...
            <div class="border-0 border-b-[1px] border-solid border-[#EBE9F1] mb-[5.6rem]">
                <div class="d-flex justify-between flex-wrap mb-17">
                    ...
                    <div class="right">
                        ...
                        <table class="table-1-el w-full">
                            <tr>
                                <td class="text-18 pb-10" style="font-weight: 500">Number #{{ quotation.code }}</td>
                            </tr>
                            <tr>
                                <td>Date: {{ $filters.formatDateTime(quotation.created_at) }}</td>
                            </tr>
                        </table>
                    </div>
                </div>
                ...
                <div class="mb-[5.6rem]">
                    ...
                    <p class="text-bold">{{ quotation.customer.name }}</p>
                    <p>{{ quotation.customer.address_location }}</p>
                    <p>{{ quotation.customer.address_postal_code }}</p>
                    <p>{{ quotation.customer.poc_email }}</p>
                </div>

                <div class="mb-[5.6rem]">
                    <div class="text-primary text-bold mb-10">Shipment</div>
                    <p>{{ quotation.shipment_text }}</p>
                </div>

                <div class="mb-[5.6rem]">
                    <div class="text-primary text-bold mb-10">Status</div>
                    <p>{{ quotation.status_text }}</p>
                </div>
            </div>
            ...
            <div class="mb-[2.6rem]">
                ...
                <div class="table-responsive pb-10">
                    <table class="table select-rows">
                        ...
                        <tbody>
                        <tr v-if="quotation.items.length > 0" v-for="(item, index) in quotation.items" :key="index">
                            <td>
                                {{ item.type }}
                            </td>
                            <td>
            ...
            <div class="row justify-end mt-[3.6rem] mb-[4rem]">
                <div class="col-lg-9">
                    <div v-if="quotation.photo" class="mb-16 border-0 border-[1px] border-solid border-[#EBE9F1] p-24 py-15 rounded-[.5rem] max-w-[50rem]">
                        <p class="text-primary text-bold-500 mb-10">Acknowledged and confirmed by:</p>
                        <img class="max-w-650 w-100 rounded-[.5rem]" :src="`/storage/${quotation.photo}`" alt="">
                    </div>
                </div>
                <div class="col-lg-3">
                    <table class="table-1-el w-full">
            ...
            <div v-if="quotation.remarks" class="mb-16 border-0 border-[1px] border-solid border-[#EBE9F1] p-24 py-15 rounded-[.5rem]">
            ...
            <div class="d-flex justify-between flex-wrap gap-[3rem]">
                <div class="btn-group mt-[5.6rem]">
                    <Link class="btn btn-purple" :href="`/${shipment}/${url}`"><i class="fa fa-chevron-left text-11"></i>Back</Link>
                    <Link class="btn btn-light btn-light--brounded" :href="`/${shipment}/${url}/${quotation.id}/edit`">Edit</Link>
                    <a class="btn btn-red btn-red--brounded" @click="destroy" href="javascript:void(0)"><i>&times;</i>Remove</a>
                </div>
                <div class="btn-group mt-[5.6rem]">
                    <input type="file"
                           ...
                           @input="form.photo = $event.target.files[0]; form.post(`/${shipment}/${url}/${quotation.id}/upload-photo`)"
                           accept="image/png, image/jpg, image/jpeg">
                    <div v-if="form.errors.photo" class="invalid-feedback d-block">{{ form.errors.photo }}</div>
                    <label class="btn btn-purple cursor-pointer" for="upload-photo">Upload</label>

                    <Link class="btn btn-green" :href="`/${shipment}/sales-order-${url}/create?quotation_id=${quotation.id}`">Convert to Sales Order</Link>
                    <a class="btn btn-orange" href="javascript:void(alert('Still wait confirmation'))">Send Quotation</a>
                </div>
            </div>
        </div>
    </Layout>
...
const destroy = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            Inertia.delete(`/${props.shipment}/${props.url}/${props.quotation.id}`, {
                preserveScroll: true,
            });
        }
    });
