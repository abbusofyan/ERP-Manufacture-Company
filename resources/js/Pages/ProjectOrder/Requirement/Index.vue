    ...
    <div class="box pb-[5.6rem] rounded-md shadow-default bg-white mt-[2.6rem]">
        <div class="flex flex-wrap gap-16 justify-between py-20 px-25 gap-1">
            ...
            <div class="flex flex-wrap gap-16 justify-between">
                ...
                <div class="btn-group">
                    <a v-if="![7, 8, 9].includes(projectOrder.status)" class="btn btn-purple" data-bs-toggle="modal" data-bs-target="#uploadRequirement" ref="openModal" href="javascript:void(0)">Add Requirement <i class="fa fa-plus"></i></a>
                </div>
            </div>
        </div>
        <div class="form-wrap">
                ...
                <div class="table-responsive pb-0" style="min-height: unset">
                    <table class="table select-rows">
                        ...
                        <tr v-for="requirement in requirements.data" :key="requirement.id">
                            ...
                            <td>
                                <div v-if="![7, 8].includes(projectOrder.status)" class="el-actions justify-end">
...
const filter = () => {
    Inertia.get(
        `/project-orders/${props.projectOrder.id}/requirements`,
        {
            search: search.value,
            order: order.value,
            by: by.value,
...
const destroy = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            Inertia.delete(`/project-orders/${props.projectOrder.id}/requirements/${id}`, {
                preserveScroll: true,
                onSuccess: () => {
                    Swal.fire({
                        title: 'Success!',
