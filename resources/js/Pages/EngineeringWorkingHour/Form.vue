<template>
    <Layout>
        <div class="flex flex-wrap items-center gap-13 mb-26">
            <div v-if="workingHour" class="big-title">Edit Engineering Working Hour</div>
            <div v-else class="big-title">Create New Engineering Working Hour</div>
            ...
            <nav class="breadcrumbs">
                <ol>
                    ...
                    <li>
                        <Link href="/engineerings">Engineerings</Link>
                    </li>
                    <li>
                        <Link href="/engineering-working-hours">Working Hours</Link>
                    </li>
                    <li>
                        <span v-if="workingHour">Edit Engineering Working Hour</span>
                        <span v-else>Create Engineering Working Hour</span>
                    </li>
                </ol>
            </nav>
        </div>
            ...
            <div class="form-wrap">
                <form @submit.prevent="handleSubmit">
                    <div class="boxes">
                        <div class="box border-0 border-b-[1px] border-solid border-[#EBE9F1] mb-[5.6rem]">
                            <div class="text-18 font-medium pb-17 mb-17 border-0 border-b-[1px] border-solid border-[#EBE9F1]">
                                ...
                                <span>Engineering Working Hour Information</span>
                            </div>
                            ...
                            <div class="grid md:grid-cols-3 gap-[7.7rem]">
                                <div class="form-group" v-if="!hasDefault || form.is_default">
                                    ...
                                    <div class="custom-checkbox style-1">
                                        ...
                                        <label for="is-default">Set as Default Engineering Working Hour</label>
                                    </div>
                    ...
                    <div class="btn-group mt-[5.6rem]">
                        ...
                        <Link class="btn btn-light btn-light--brounded" href="/engineering-working-hours">Discard</Link>
                    </div>
...
watch(() => form.is_lunch_time, (newVal) => {
    if (newVal) {
...
watch(() => form.is_dinner_time, (newVal) => {
    if (newVal) {
        form.is_default = false;
        form.is_lunch_time = false;
        form.specific_date = null;
...
const handleSubmit = () => {
    if (validateTime()) {
        if (props.workingHour) {
            form.put(`/engineering-working-hours/${props.workingHour.id}`);
        } else {
            form.post('/engineering-working-hours');
        }
    } else {
        Swal.fire({
            title: 'Error',
