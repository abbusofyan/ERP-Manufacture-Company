...
class ServiceWorkingHourController extends Controller
{
    public function index(Request $request)
    {
        ...
        $defaultWorkingHours = ServiceWorkingHour::where('is_default', true)->first();
        $defaultLunchTime = ServiceWorkingHour::where('is_lunch_time', true)->first();
        $defaultDinnerTime = ServiceWorkingHour::where('is_dinner_time', true)->first();
        ...
        foreach ($dates as $date) {
            ...
            $specificWH = ServiceWorkingHour::where('specific_date', $dateStr)->first();
        ...
        $defaultTimes = ServiceWorkingHour::where('is_default', true)
            ->orWhere('is_lunch_time', true)
            ->orWhere('is_dinner_time', true)
            ->get();

        return Inertia::render('ServiceWorkingHour/Index', [
    ...
    public function create(Request $request): Response
    {
        ...
        return Inertia::render('ServiceWorkingHour/Form', [
            'date' => $date,
            'default' => ServiceWorkingHour::where('is_default', true)->first(),
            'hasDefault' => ServiceWorkingHour::where('is_default', true)->exists(),
            'hasLunchTime' => ServiceWorkingHour::where('is_lunch_time', true)->exists(),
            'hasDinnerTime' => ServiceWorkingHour::where('is_dinner_time', true)->exists(),
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        foreach ($flags as $flag) {
            if ($request->has($flag) && $request->$flag) {
                $defaultExists = ServiceWorkingHour::where($flag, true)->exists();
                if ($defaultExists) {
        ...
        ServiceWorkingHour::create($data);

        return redirect()->route('service-working-hours.index')->with('created', 'Working hours created successfully');
    }

    public function edit(ServiceWorkingHour $service_working_hour): Response
    {
        return Inertia::render('ServiceWorkingHour/Form', [
            'hasDefault' => ServiceWorkingHour::where('is_default', true)->exists(),
            'hasLunchTime' => ServiceWorkingHour::where('is_lunch_time', true)->exists(),
            'hasDinnerTime' => ServiceWorkingHour::where('is_dinner_time', true)->exists(),
            'workingHour' => $service_working_hour
        ]);
    }

    public function update(Request $request, ServiceWorkingHour $service_working_hour): Redirector|RedirectResponse|Application
    {
        $request->validate([
            'start_time' => 'required|array',
            'end_time' => 'required|array',
        ...
        foreach ($flags as $flag) {
            if ($request->has($flag) && $request->$flag) {
                $defaultExists = ServiceWorkingHour::whereNull('specific_date')
                    ->where($flag, true)
                    ->where('id', '!=', $service_working_hour->id)
                    ->exists();
                if ($defaultExists) {
                    return redirect()->back()->withErrors(['start_time' => "A default {$flag} already exists."]);
                }
                $data['specific_date'] = null;
                ServiceWorkingHour::whereNull('specific_date')
                    ->where($flag, true)
                    ->where('id', '!=', $service_working_hour->id)
                    ->delete();
            }
        ...
        $service_working_hour->update($data);

        return redirect()->route('service-working-hours.index')->with('updated', 'Working hours updated successfully');
    }

    public function destroy(ServiceWorkingHour $service_working_hour): RedirectResponse
    {
        $service_working_hour->delete();
