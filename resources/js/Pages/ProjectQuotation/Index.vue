        ...
        <div class="box rounded-md shadow-default bg-white">
            <div class="flex flex-wrap gap-16 justify-between py-20 px-25 gap-1">
                ...
                <div class="flex flex-wrap gap-16 justify-between">
                    ...
                    <div class="btn-group">
                        <Link class="btn btn-purple" href="/project-quotations/create">+ Create New</Link>
                    </div>
                </div>
            </div>
            <div class="table-responsive">
                <table class="table select-rows">
                    ...
                    <tbody>
                    <tr v-for="quotation in projectQuotations.data" :key="quotation.id">
                            ...
                            <div class="more">
                                ...
                                <ul class="more-panel">
                                    <li v-if="permissions.includes('view-requisition')">
                                        <Link :href="`/project-quotations/${quotation.id}`"><span class="icon"><img src="../../../assets/img/documents.svg" alt=""></span><span>View Detail</span></Link>
                                    </li>
                                    <li v-if="permissions.includes('update-requisition') && quotation.invoices && quotation.invoices.length == 0">
                                        <Link :href="`/project-quotations/${quotation.id}/edit`"><span class="icon"><img src="../../../assets/img/edit.svg" alt=""></span><span>Edit</span></Link>
                                    </li>
                                    <li v-if="permissions.includes('view-requisition') && quotation.status == 2">
                                        <Link :href="`/project-quotations/${quotation.id}/generate-proforma-invoice`"><span class="icon"><img src="../../../assets/img/download.svg" alt=""></span><span class="text-nowrap">Generate Proforma Invoice</span></Link>
                                    </li>
                                    <li v-if="permissions.includes('view-requisition') && quotation.status == 2">
                                        <Link :href="`/project-quotations/${quotation.id}/generate-invoice`" :disabled="!quotation.vehicle_parts.some(part => part.project_invoice_id === null)"><span class="icon"><img src="../../../assets/img/download.svg" alt=""></span><span>Generate Invoice</span></Link>
                                    </li>
                                    ...
                                    <li v-if="permissions.includes('update-requisition') && quotation.status != 3 && (quotation.project_order == null || quotation.project_order.status == 9)">
            ...
            <div class="flex items-center justify-between gap-26 mt-25 pb-25 px-25">
                <p>Showing {{ projectQuotations.from }} to {{ projectQuotations.to }} of {{ projectQuotations.total }} entries</p>
                <Pagination :links="projectQuotations.links"/>
            </div>
        </div>
    </Layout>
</template>
...
const submitImage = (id) => {
    form.post(`/project-quotations/${id}/upload-photo`, {
        onSuccess: () => {
            form.reset();
            Swal.fire({
                title: 'Success !',
...
const voidChange = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            Inertia.post(`/project-quotations/${id}/void`, {}, {
                onSuccess: () => {
                    form.reset();
                    Swal.fire({
                        title: 'Success !',
...
const destroy = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            Inertia.delete(`/project-quotations/${id}`, {
                preserveScroll: true,
                onSuccess: () => {
                    Swal.fire({
                        title: 'Success!',
